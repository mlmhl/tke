...
export class DateTimePicker extends React.Component<DateTimePickerProps, any> {
  constructor(props: DateTimePickerProps) {
    ...
    const { dateFrom, timeFrom, dateTo, timeTo, rangeFrom, rangeTo, pickerValue } = this.getInitDateAndRange(
      this.props
    );
    this.state = {
      active: false,
   ...
   */
  getInitRange = props => {
    let { duration, range } = props;
    range = range || {};

    let rangeFrom: DateTimeRange = {},
      rangeTo: DateTimeRange = {};
  ...
  render() {
    ...
    // picker内容显示
    let text = this.props.placeHolder || t('选择日期');
    if (pickerValue) {
      text = this.format(pickerValue.from) + t(' 至 ') + this.format(pickerValue.to);
    }
    ...
    const tabs = this.props.tabs
      ? this.props.tabs.map((tab, index) => {
          if (selectedTabIndex !== index) {
            return (
              <span role="tab" tabIndex={index} key={index} onClick={() => this.handleTabSelect(tab, index)}>
                {tab.label}
              </span>
            );
          } else {
            return (
              <span
                ...
                key={index}
                className="current"
                onClick={() => this.handleTabSelect(tab, index)}
              >
                {tab.label}
              </span>
            );
    ...
    return (
      <div className="tc-15-calendar-select-wrap tc-15-calendar2-hook">
        ...
        <div
          className={classNames('tc-15-dropdown tc-15-dropdown-btn-style date-dropdown', {
            'tc-15-menu-active': this.state.active,
            disabled: this.props.disabled
          })}
        >
          <a className="tc-15-dropdown-link" onClick={this.open} onFocus={this.open}>
            <i className="caret" />
            {text}
          </a>
          <div className="tc-15-dropdown-menu" role="menu">
              ...
              <div className="custom-date-wrap">
                ...
                <div className="calendar-box">
                  <SingleDatePicker
                    value={dateFrom}
                    range={{
                      min: this.formatDate(this.getDate(rangeFrom.min)),
                      max: this.formatDate(this.getDate(rangeFrom.max))
                    }}
                    onChange={dateFrom => {
                      ...
                      if ('duration' in this.props) {
                        ...
                        if (this.compare(to, rangeTo.max) > 0) {
                          ...
                          this.setState({
                            dateFrom: this.formatDate(this.getDate(from)),
                            timeFrom: this.formatTime(this.getTime(from)),
                            dateTo: this.formatDate(this.getDate(rangeTo.max)),
                            timeTo: this.formatTime(this.getTime(rangeTo.max))
                          });
                        } else {
                          this.setState({
                            dateFrom,
                  ...
                  <TimePicker
                    ...
                    onChange={timeFrom => {
                      if (dateFrom) {
                        ...
                        if ('duration' in this.props) {
                          const to = new Date(
                            this.parse(`${dateFrom} ${timeFrom}`).getTime() + this.props.duration * unit
                          );
                          this.setState({
                            dateTo: this.formatDate(this.getDate(to)),
                            timeTo: this.formatTime(this.getTime(to))
              ...
              <div className="custom-date-wrap">
                ...
                <div className="calendar-box">
                  <SingleDatePicker
                    ...
                    range={{
                      min: this.formatDate(this.getDate(rangeTo.min)),
                      max: this.formatDate(this.getDate(rangeTo.max))
                    }}
                    onChange={dateTo => {
            ...
            <div className="custom-date-ft">
              <button type="button" className="tc-15-btn m" onClick={this.handleSubmit}>
                {t('确定')}
              </button>
              &nbsp;
              <button type="button" className="tc-15-btn m weak" onClick={this.handleCancel.bind(this)}>
                {t('取消')}
              </button>
            </div>
          </div>
