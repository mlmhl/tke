...
import { getBackendsGroupYamlContent } from '../../../services/api';
...
class YamlPanel extends React.Component<PropTypes, StateTypes> {
  ...
  loadData = async () => {
    let { clusterName, namespace, name } = this.props;
    await this.getContent(clusterName, namespace, name);
  }
  /**
   * 切换"对象选择"
   * @param source
  ...
  getContent = async (clusterName, namespace, backendsGroupName) => {
    let yamlContent = await getBackendsGroupYamlContent(clusterName, namespace, backendsGroupName);
