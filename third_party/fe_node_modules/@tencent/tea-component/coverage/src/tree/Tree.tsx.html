...
<html lang="en">
<head>
    <title>Code coverage report for src/tree/Tree.tsx</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../index.html">All files</a> / <a href="index.html">src/tree</a> Tree.tsx
    </h1>
    <div class='clearfix'>
...
<span class="cstat-no" title="statement not covered" >      nodes.push(pid);</span>
    }
  }
  // 插入缺失的子节点
  for (const id of nodes) {
    const children = [...(childrenMap.get(id) || [])];
    children.forEach(<span class="fstat-no" title="function not covered" >ci</span>d =&gt; {
<span class="cstat-no" title="statement not covered" >      if (!nodes.includes(cid)) {</span>
<span class="cstat-no" title="statement not covered" >        nodes.push(cid);</span>
      }
    });
  }
  return nodes;
...
function getLeafIds(
  ...
  const nodes = [];
  all.forEach(id =&gt; {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!childrenMap.get(id)) {
      nodes.push(id);
    }
  });
  return nodes;
...
&nbsp;
  React.Children.forEach&lt;React.ReactNode&gt;(children, child =&gt; {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (isTreeNode(child)) {
      const { id, selectable, disableSelect } = child.props;
      <span class="missing-if-branch" title="else path not taken" >E</span>if (selectable !== false) {
        // 非最外层根
        if (rootId) {
          relations[id] = rootId;
        ...
        // 禁用
        <span class="missing-if-branch" title="if path not taken" >I</span>if (disableSelect) {
<span class="cstat-no" title="statement not covered" >          disabledIds.push(id);</span>
        }
      }
      // 当前结点不可选时其子结点 rootId 为当前 rootId
      traverse(selectable !== false ? id : <span class="branch-1 cbranch-no" title="branch not covered" >rootId,</span> child.props.children, tree);
    } else <span class="cstat-no" title="statement not covered" >if (React.isValidElement(child)) {</span>
      // eslint-disable-next-line dot-notation
<span class="cstat-no" title="statement not covered" >      traverse(rootId, child.props["children"], tree);</span>
    }
  });
}
&nbsp;
...
export function TreeView({
  ...
  expandedIds,
  defaultExpandedIds = [],
  onExpand = <span class="fstat-no" title="function not covered" >()</span> =&gt; <span class="cstat-no" title="statement not covered" >null,</span>
  onLoad,
  onLoadError,
  switcherIcon,
  className,
...
&nbsp;
  return (
    &lt;div className={classNames(`${classPrefix}-tree`, className)} style={style}&gt;
      &lt;TreeContext.Provider
        value={{
          ...
          onActive: <span class="fstat-no" title="function not covered" >ac</span>tiveId =&gt;
<span class="cstat-no" title="statement not covered" >            setActiveNodes([activeId], { active: true, nodeId: activeId }),</span>
          onExpand: <span class="fstat-no" title="function not covered" >(n</span>odeId, expanded) =&gt;
<span class="cstat-no" title="statement not covered" >            setExpandedNodes(</span>
              expanded
                ? [...expandedNodes, nodeId]
                : expandedNodes.filter(<span class="fstat-no" title="function not covered" >id</span> =&gt; <span class="cstat-no" title="statement not covered" >id !== nodeId)</span>,
              { nodeId, expanded }
            ),
          onLoad,
          onLoadError,
        ...
        &lt;ul&gt;
          {selectable ? (
            &lt;CheckTree
              ...
              onChange={<span class="fstat-no" title="function not covered" >(v</span>alue, { check }) =&gt;
<span class="cstat-no" title="statement not covered" >                setSelectedNodes(</span>
