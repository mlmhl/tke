...
function TabBar(_a) {
    var activeId = _a.activeId, tabs = _a.tabs, _b = _a.onActive, onActive = _b === void 0 ? function () { return null; } : _b, addon = _a.addon, vertical = _a.vertical, maxHeight = _a.maxHeight, tabBarRender = _a.tabBarRender, style = _a.style;
    var classPrefix = config_context_1.useConfig().classPrefix;
    // 插件区域宽度
    var addonsRef = react_1.useRef(null);
    var _c = tslib_1.__read(react_1.useState(0), 2), addonWidth = _c[0], setAddonWidth = _c[1];
    ...
    var _d = tslib_1.__read(react_1.useState(0), 2), offset = _d[0], setOffset = _d[1];
    var _e = tslib_1.__read(react_1.useState(false), 2), scrolling = _e[0], setScrolling = _e[1];
    ...
    return (react_1.default.createElement("div", { className: classPrefix + "-tabs__tabbar", style: style },
        react_1.default.createElement("div", { ref: scrollAreaRef, className: classnames_1.default(classPrefix + "-tabs__scroll-area", {
                "is-scrolling": scrolling,
            }), style: {
                maxHeight: vertical ? maxHeight : undefined,
                marginRight: addonWidth,
            } },
            react_1.default.createElement("ul", { ref: tabListRef, className: classPrefix + "-tabs__tablist", style: {
                    transition: "transform ease-out .2s",
                    transform: vertical
                        ? "translate3d(0, " + offset + "px, 0)"
                        : "translate3d(" + offset + "px, 0, 0)",
                } }, tabs.map(function (tab) { return (react_1.default.createElement(TabItem_1.TabItem, { ref: tab.id === activeId ? activeItemRef : undefined, key: tab.id, label: tab.label, actived: tab.id === activeId, disabled: tab.disabled, onClose: tab.onClose ? function (evt) { return tab.onClose(tab, evt); } : null, onClick: function (evt) { return onActive(tab, evt); }, render: function (children) { return tabBarRender(children, tab); } })); })),
            react_1.default.createElement(button_1.Button, { ref: buttonRef, className: classPrefix + "-tabs__backward", type: "icon", icon: vertical ? "arrowup" : "arrowleft", disabled: offset >= 0, onClick: handleBackward }),
            react_1.default.createElement(button_1.Button, { className: classPrefix + "-tabs__forward", type: "icon", icon: vertical ? "arrowdown" : "arrowright", disabled: offset <= 0 - getMaxOffset(), onClick: handleForward })),
        react_1.default.createElement("div", { className: classPrefix + "-tabs__addons", ref: addonsRef }, addon)));
}
