...
function expandable(options) {
    ...
    return {
            ...
            if (recordIndex !== -1) {
                ...
                if (isExpandable) {
                    ...
                    var element = (react_1.default.createElement("div", null,
                        react_1.default.createElement(IconWrap, { onClick: toggle },
        ...
        onInjectRow: function (renderRow) { return function (record, rowKey, recordIndex, columns) {
            var key = getRowKey(record, recordIndex);
            var isExpandable = !shouldRecordExpandable || shouldRecordExpandable(record);
            var onClick = function (e) { return e; };
            if (rowExpand && isExpandable) {
                var isExpanded_2 = expandedKeySet.has(key);
            ...
            if (!expandedKeySet.has(rowKey)) {
                var _a = renderRow(record, rowKey, recordIndex, columns), prepends_1 = _a.prepends, row_1 = _a.row, appends_1 = _a.appends;
                return {
                    prepends: prepends_1,
                    row: react_1.default.cloneElement(row_1, {
                        onClick: function (event) {
                            if (typeof row_1.props.onClick === "function") {
                                row_1.props.onClick(event);
                            }
                            onClick(event);
                        },
                    }),
            ...
            // 原始行的渲染内容
            var _b = renderRow(record, rowKey, recordIndex, columns), prepends = _b.prepends, row = _b.row, appends = _b.appends;
            // 展开的内容
            var expands = [];
            ...
            return {
                ...
                row: react_1.default.cloneElement(row, {
                    ...
                    onClick: function (event) {
                        if (typeof row.props.onClick === "function") {
...
function IconWrap(_a) {
    var onClick = _a.onClick, children = _a.children;
    var classPrefix = config_context_1.useConfig().classPrefix;
    return (react_1.default.createElement("a", { className: classPrefix + "-table__icon-wrap", onClick: onClick }, children));
}
