...
function sortable(options) {
    ...
    return {
        onInjectColumn: function (renderColumn) { return function (record, rowKey, recordIndex, column, columnIndex) {
            var columnResult = renderColumn(record, rowKey, recordIndex, column, columnIndex);
            ...
            var changeOrder = function (event) {
                ...
                var nextOrder;
                var prefer = sortablePrefer.get(column.key);
                if (!order) {
                    nextOrder = prefer;
                }
                if (order === "asc") {
                ...
                var value = (nextOrder ? [sort] : []).concat(Array.from(sortMap).map(function (_a) {
            ...
            return tslib_1.__assign(tslib_1.__assign({}, columnResult), { children: children });
        }; },
    };
}
...
function SortButton(_a) {
    ...
    return (react_1.default.createElement("span", { className: classnames_1.default(classPrefix + "-table__sortbtn", { "is-active": Boolean(order) }, className), style: style, onClick: onClick },
        react_1.default.createElement(text_1.Text, { overflow: true, className: classPrefix + "-table__headerop" }, children),
