...
export class ChartInstancesPanel extends React.Component<ChartInstancesPanelProps, ChartInstancesPanelState> {
  ...
  checkLineIsDisable(instanceList = [], key = "") {
    ...
    const instance = instanceList.find(instance => {
      const instanceKey = CHART_PANEL.GenerateRowKey(this.props.groupBy.map(groupBy => instance[groupBy.value]));
  ...
  onCheckInstances = (instanceList: Array<any>) => {
    this.setState({ instanceList }, () => {
      ...
      const seriesGroup = seriesGroupTemp.map((series: CHART_PANEL.ChartDataType) => {
          // series 是每一个图表的数据
  ...
  requestData(tableInfo: any = {}) {
    ...
    return CHART_PANEL.RequestData({
      ...
      .then(res => {
         ...
         */
        const dimensionsData = CHART_PANEL.AggregateDataByDimensions(groupByItems, columns, data);
          ...
          <div className="tc-g">
            <div className="tc-g-u-3-4">
              <MetricCharts
                ...
                reload={this.fetchDataByTables.bind(this, { instances: this.state.instanceList })}
              />
            </div>
