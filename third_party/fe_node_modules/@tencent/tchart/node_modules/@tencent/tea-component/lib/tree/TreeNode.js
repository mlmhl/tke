...
function TreeNodeInner(_a) {
    var id = _a.id, content = _a.content, icon = _a.icon, operation = _a.operation, selectable = _a.selectable, activable = _a.activable, activeIds = _a.activeIds, onActive = _a.onActive, expandable = _a.expandable, children = _a.children, expandedIds = _a.expandedIds, onExpand = _a.onExpand, onLoad = _a.onLoad, onLoadError = _a.onLoadError, switcherIcon = _a.switcherIcon, className = _a.className, style = _a.style;
    var expanded = expandedIds.includes(id);
    var nodeIcon = inject_value_1.injectValue(icon)({ expanded: expanded });
    return (react_1.default.createElement("li", { className: classnames_1.default("tea-tree__node", className, {
            // 高亮
            "is-selected": activable && activeIds.includes(id),
        }), style: style },
        react_1.default.createElement("div", { className: "tea-tree__node-content", style: activable ? { cursor: "pointer" } : undefined, onClick: function () { return activable && onActive(id); } },
            react_1.default.createElement(TreeNodeSwitcher, { id: id, icon: switcherIcon, enable: Boolean(expandable || children), expanded: expanded, onExpand: onExpand, onLoad: onLoad, onLoadError: onLoadError }),
            react_1.default.createElement("div", { className: "tea-tree__label" }, selectable ? (react_1.default.createElement(check_1.Check, { name: id, type: "checkbox" },
                nodeIcon,
                content)) : (react_1.default.createElement(react_1.default.Fragment, null,
                nodeIcon,
                react_1.default.createElement("span", { className: "tea-tree__label-title" }, content)))),
            operation && react_1.default.createElement("div", { className: "tea-tree__action" }, operation)),
        expanded && react_1.default.createElement("ul", { className: "tea-tree__subtree" }, children)));
}
...
function TreeNodeSwitcher(_a) {
    ...
    var switcherIcon = inject_value_1.injectValue(icon)({ expanded: expanded });
    ...
    return (react_1.default.createElement("a", { href: "javascript:;", className: "tea-tree__switcher" }, enable && switcher));
}
function ActionLink(_a) {
    var children = _a.children, className = _a.className, props = tslib_1.__rest(_a, ["children", "className"]);
    return (react_1.default.createElement("a", tslib_1.__assign({ className: classnames_1.default("tea-text--link", className) }, props), children));
}
