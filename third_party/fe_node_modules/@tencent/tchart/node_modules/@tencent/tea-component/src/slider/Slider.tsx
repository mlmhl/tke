...
export function Slider({ min = 0, max, ...props }: SliderProps) {
  ...
  const proportion = ((current - min) / range) * 100;
  ...
  return (
    <div
      className={classNames("tea-slider", className, {
    ...
    >
      <Popover
        ...
        overlay={({ placement, scheduleUpdate }) => {
          ...
          return (
            <BubbleContent placement={placement}>
              {tipFormatter(getValueByPosition(mousePosition))}
            </BubbleContent>
          );
        }}
      >
        <div
          className="tea-slider__content"
        ...
        >
          ...
          {/* 滑轨 */}
          <div className="tea-slider__bar">
            <div
              className="tea-slider__track"
          ...
          >
            ...
            >
              <span className="tea-slider__handle-inner" />
            </div>
          </Popover>

          {/* 标尺 */}
          <div className="tea-slider__mark">
            {marks
              .filter(({ value }) => value >= min && value <= max)
              .map(({ value, label }) => (
                <span
                  key={value}
                  className="tea-slider__text"
                  style={{
      ...
      <div className="tea-slider__extend">{after}</div>
    </div>
  );
}
