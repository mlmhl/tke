...
export function CheckTree(props: CheckTreeProps) {
  ...
  const model = useTreeModel(relations, value, disabledSet, shiftSelect);
...
function useTreeModel(
  relations: CheckTreeRelation,
  selection: string[],
  disabledSet: Set<string>,
  shiftSelect: boolean
) {
  // prev record for shift select
  const [prevCheck, setPrevCheck] = useState<{ level: number; index: number }>(
    null
  );
  ...
  function visit(check: CheckProps) {
    ...
    // 每有新结点加入重新计算
    const roots = visited.filter(check => !parentMap.has(check.name));

    if (roots.length) {
      let curLevel = 0;
      levelMap = [roots.map(c => c.name)];
      const nameSet = new Set<string>(roots.map(c => c.name));
  ...
  function handleChange(
    ...
    if (shiftSelect && event && isEventWithShiftKey(event) && event.shiftKey) {
      if (prevCheck !== null) {
        const { level: prevLevel, index: prevIndex } = prevCheck;
        if (level === prevLevel) {
          // 清空文字选区
