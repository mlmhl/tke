...
import { CheckContext, CheckProps } from "../../check";
  ...
  return {
    ...
    onInjectColumn: previous => (record, rowKey, recordIndex, column) => {
      ...
      // 记录行
        ...
        const checkbox = (
          <Checkbox className="tea-form-check--table-select" name={rowKey}>
            {children || <Fragment>&nbsp;</Fragment>}
          </Checkbox>
        );
    ...
    onInjectTable: renderBody => props => {
      ...
      const disabledKeys = Array.from(
        new Set([...Object.keys(relations || {}), ...records.map(rowKey)])
      ).filter(
        key =>
          !rowSelectable(key, {
